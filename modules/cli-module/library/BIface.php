...
class BIface
{
    static function build(string $here, string $name): bool{
        ...
        $lib_name = to_ns($name);
        $lib_ns   = to_ns($mod_name . '\\Iface');
        $lib_file = 'modules/' . $mod_name . '/interface/' . $lib_name . '.php';
        
        if(is_file($lib_file))
            Bash::error('Interface with the same file name already exists');
        
        $lib_config = BClass::getConfig($lib_ns, $lib_name, null, 'interface');
        BClass::write($here, $mod_conf, $lib_config, $lib_file, 'interface');
